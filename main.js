/**
 * NetLogic Replace - Electron Main Process
...
function createTray() {
  ...
  const menu = Menu.buildFromTemplate([
    { label: 'Show NetLogic Replace', click: () => { mainWindow?.show(); } },
    { type: 'separator' },
    { label: 'Quit', click: () => { app.isQuitting = true; app.quit(); } },
  ]);
  tray.setToolTip('NetLogic Replace');
...
async function exportReport(event, { format, data }) {
  ...
  const result = await dialog.showSaveDialog(mainWindow, {
    title: 'Save NetLogic Replace Report',
...
<title>NetLogic Replace Report — ${data.target}</title>
<style>
...
<h1>NetLogic Replace Security Report</h1>
...
<p style="color:#7d8590;font-size:.8rem;margin-top:2rem">Generated by NetLogic Replace — authorized use only.</p>
</body></html>`;
}
