/**
 * NetRecon - Electron Main Process
...
function getPythonPath() {
  ...
  const bundled = path.join(
    ...
    'python_dist',
    IS_WIN ? 'netrecon_engine.exe' : 'netrecon_engine'
  );
  if (fs.existsSync(bundled)) return { exe: bundled, script: null };

  // 2. System Python with script
  const scriptPath = path.join(__dirname, '..', 'netrecon.py');
...
function createTray() {
  ...
  const menu = Menu.buildFromTemplate([
    { label: 'Show NetRecon', click: () => { mainWindow?.show(); } },
    { type: 'separator' },
    { label: 'Quit', click: () => { app.isQuitting = true; app.quit(); } },
  ]);
  tray.setToolTip('NetRecon');
...
async function exportReport(event, { format, data }) {
  ...
  const result = await dialog.showSaveDialog(mainWindow, {
    title: 'Save NetRecon Report',
    defaultPath: `netrecon_${data.target}_${Date.now()}.${format}`,
...
<title>NetRecon Report — ${data.target}</title>
<style>
...
<p style="color:#7d8590;font-size:.8rem;margin-top:2rem">Generated by NetRecon — authorized use only.</p>
</body></html>`;
}
